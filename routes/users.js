...
function GetTime(){
  ...
  let sum =today.getHours()*60 + today.getMinutes()
  minute = pluszero(minute)
  hour = pluszero(hour)
  let latest = String(hour)+String(minute)
  let sec = today.getSeconds()
  return ([latest,sum,sec])
}
...
router.route('/add').post((req, res) => {
  ...
  const quest =[{login: 1,work: 0,hours: 0,hours2:0,review: 0,plan: 0,message: 0},{rest: 0,rest2:0,fav: 1}]
  const status = "before"
  const hours = {work: {start: [], end:[]},rest:[],resttime:0}
  const items ={cards:[{name: "Boi", hp: 1200, ap: 200,ap2:400,cost:1,buff:0,equip:1}],
  buffs:[{name: "Health Potion",fname: "Common", power: 100, type: "HP"}],deck:[]}
...
router.route('/getRanking').get((req, res) => {
...
router.route('/reset').post((req, res) => {
  const quest2 =[{login: 1,work: 0,hours: 0,hours2:0,review: 0,message: 0},{rest: 0,hours: 0,review: 0,plan: 0}]
  User.updateOne({username: req.body.username},{"$set":{"point.exp": [0,0], status: "before", quest: quest2, hours:{work:{start:[],end:[]},rest:[],resttime:0}}})
    .then(() => {
      if(req.body.monday){
        User.updateOne({username: req.body.username},{"$set":{"quest.1":{rest:0,fav:1}}}).then(()=>res.json("Reset!").catch(err => res.status(400).json('Error: ' + err)))
      } else res.json("Reset!")
    })
    .catch(err => res.status(400).json('Error: ' + err));
});
